...
# extract 함수: FRED API에서 PPIACO 데이터 추출
def extract(**kwargs):
    ...
    ppi_df = pd.DataFrame({
        "date": ppi_data.index,
        "producer_price_index": ppi_data.values  # 컬럼명 변경: consumer_price_index -> producer_price_index
    })

    # S3로 CSV 저장
    upload_csv_to_S3(ppi_df, True, ds_nodash)
...
# DAG 정의
dag = DAG(
    dag_id='PPIACO',  # DAG 이름도 PPIACO로 변경
